...
{
    string ip = DistributedRPC::GetInstance().GetConfig().Load("rpcserverip");
    uint16_t port=atoi(DistributedRPC::GetInstance().GetConfig().Load("rpcserverport").c_str());
                            ...
                            muduo::Timestamp)
{
    ...
    //根据 header_size读取数据头的原始字符流，反序列化数据，得到rpc请求的详细信息
    std::string rpc_header_str= recv_buf.substr(4,header_size);
    distributedrpc::RpcHeader rpcHeader;
...
void RpcProvider::SendRpcResponse(const muduo::net::TcpConnectionPtr& conn, google::protobuf::Message *response)
{
    ...
    {
        // 序列化成功后， 通过网络把rp方法执行的结果发送回rpc的调用方
